...
# ═══════════════════════════════════════════════════════════════════════
#   INSTALLATEUR ZITELOU POUR macOS
...
if command -v adb &> /dev/null; then
    ...
    if ! command -v brew &> /dev/null; then
        print_warning "Homebrew n'est pas installé. Installation de Homebrew..."
        echo ""
        ...
        if [ $? -ne 0 ]; then
            print_error "L'installation de Homebrew a échoué"
            echo ""
            print_info "Installez manuellement Homebrew depuis : https://brew.sh"
            exit 1
        fi
    ...
    echo ""
    echo "   📥 Installation d'ADB via Homebrew..."
...
if [ "$DEVICE_COUNT" -eq 0 ]; then
    ...
    echo ""
    print_info "NOTE POUR macOS :"
    echo ""
    echo "   → Vous devrez peut-être autoriser l'accès USB dans"
